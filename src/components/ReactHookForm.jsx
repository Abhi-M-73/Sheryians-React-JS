...
const ReactHookForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit(submitHandler)} className='flex flex-col gap-5 w-96 p-8 bg-gray-700 shadow-lg rounded'>
      ...
      <input {...register("username", { required: "Username is required" })} type="text" placeholder='Username' className='rounded p-2 bg-gray-800' />
      {
        errors.username && <h1 className='text-red-500 font-bold mt-1'>{errors.username.message}</h1>
      }
      <input {...register("email", { required: "Email is required" })} type="email" placeholder='email' className='rounded p-2 bg-gray-800' />
      {
        errors.email && <h1 className='text-red-500 font-bold mt-1'>{errors.email.message}</h1>
      }
      <input {...register("password", { required: "Password is required" })} type="text" placeholder='Password' className='rounded p-2 bg-gray-800' />
      {
